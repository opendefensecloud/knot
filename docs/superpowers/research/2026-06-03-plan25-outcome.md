...
| 49d6563, 36825c7, 456a310 | T6 | public board SVG endpoint (`/p/:token/boards/:id/svg`) + review fixes |
| 4da7ebe | T7+T8 | Excalidraw lazy chunk + yBinding.ts (Option A) |
| b4664ce | T9 | BoardEditor (full-screen modal with WS provider) |
| 8d438b3 | T10 | BoardInline (inline preview component, cached SVG) |
| 094c206 | T11 | ExcalidrawBoardNode (Tiptap node + NodeView) |
| e14db66 | T12 | slash command + insertBoard editor command |
| 4247d77 | T13 | markdown sentinel round-trip (`<!--knot:board:ID-->` image) |
| db04aa2 | T14 | deprecated `/collab/:doc_id` alias removed; KnotProvider URL switched to `/collab/doc/:id` |
| (this) | T15 | outcome doc + README row |

## Gates

**Verified:**

- `cargo test -p knot_storage` — PgBoardStore round-trip + snapshot/update reads (T2)
- `cargo test -p knot_crdt` — BoardRoom actor apply-update + snapshot persistence (T3)
- `cargo test -p knot_server` — boards REST handlers + public SVG route (T5, T6)
...
**Inline vs modal split.** `BoardInline` renders the cached SVG with a click-to-edit affordance and never loads the Excalidraw chunk. `BoardEditor` (full-screen modal) lazy-imports Excalidraw + opens a WS provider for the board. Reading a page with N boards costs N SVG fetches, zero JS chunks. Editing one costs the Excalidraw chunk load on first open.

**Markdown round-trip.** The sentinel `![](knot:board:<ID>)` with HTML comment `<!--knot:board:<ID>-->` survives serialize/parse. Sentinel detection requires that the paragraph contain *exactly* one image with a `knot:board:` URL — mixed-content paragraphs fall back to a regular image node to avoid surprising users.
...
**4. Mixed-content paragraphs containing a sentinel are rejected.** A paragraph like `text ![](knot:board:xxx) more text` does *not* round-trip as a board node — only "paragraph wraps exactly one sentinel image" triggers `excalidraw_board`. This keeps the boundary tight and avoids silent reflow of user prose.

**5. `make schema.gen` is the canonical schema regen path.** Both `crates/knot_schema/src/lib.rs` and `web/src/schema.ts` are generated; hand-editing either side gets clobbered. T11 + T13 both went through this step.

**6. `apiFetch` is JSON-only, so `boards.api.ts` uses direct `fetch` for SVG GET/PUT.** Mirrors `blobs.api.ts` for the same reason — content-type isn't JSON, and the response body is an SVG string, not a parsed object.
...
**8. The deprecated `/collab/:doc_id` alias was removed in the same cycle as the frontend URL switch.** T14 dropped both the server route alias and the client's old URL string. There is no compat window — single source of truth, fewer footguns later.

**9. Test infra: `knot_test_support::fresh_db` against dev-compose.** Per project rule, never `testcontainers::Postgres` — it caused OOMs from leaked containers twice. All `knot_storage` board tests use `fresh_db`.

**10. Excalidraw lazy chunk is ~1.1 MB gz, separate bundle.** Pages without boards never load it. The first board edit on a page triggers a single chunk fetch; subsequent opens are cached. Inline preview (`BoardInline`) never pays this cost because it only renders the cached SVG.
...
| Path | Role |
|---|---|
| `migrations/*plan25*` | boards / board_updates / board_snapshots schema (T1) |
| `crates/knot_storage/src/board_store.rs` | BoardStore trait + PgBoardStore impl |
| `crates/knot_crdt/src/board_room.rs` | BoardRoom actor |
| `crates/knot_crdt/src/board_rooms.rs` | BoardRooms registry |
| `crates/knot_server/src/routes/collab.rs` | `/collab/doc/:id` + `/collab/board/:id` WS routes |
| `crates/knot_server/src/routes/boards.rs` | boards REST + SVG cache |
| `crates/knot_server/src/routes/public.rs` | `/p/:token/boards/:id/svg` |
| `web/src/features/boards/yBinding.ts` | Option A Y binding (whole-scene replace) |
| `web/src/features/boards/BoardEditor.tsx` | full-screen modal with WS provider |
| `web/src/features/boards/BoardInline.tsx` | inline cached-SVG preview |
| `web/src/features/boards/boards.api.ts` | direct-fetch SVG GET/PUT + JSON metadata |
| `web/src/features/editor/nodes/ExcalidrawBoardNode.ts` | Tiptap node + NodeView |
| `web/src/features/editor/slash/insertBoard.ts` | slash command + editor command |
| `crates/knot_md/src/board.rs` | sentinel round-trip parse/serialize |
